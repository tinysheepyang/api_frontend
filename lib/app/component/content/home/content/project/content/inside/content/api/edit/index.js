(function () {
    ...
    angular.module('danlan').config(['$stateProvider', 'RouteHelpersProvider', function ($stateProvider, helper) {
        $stateProvider.state('home.project.inside.api.edit', {
    ...
    function indexController($scope, $rootScope, ApiManagementResource, $state, GroupService, $filter, CODE, HTTP_CONSTANT, HTML_LAZYLOAD) {
        ...
        vm.data.assistantFun.confirm = function () {
            ...
            var i = 0,
                j = 0;
            vm.check = false;
            for (i = template.apiHeader.length - 1; i >= 0; i--) {
                //请求头部 Request head
            ...
            if (!vm.check) {
                for (i = 0; i < template.apiRequestParam.length; i++) {
            ...
            if (!vm.check) {
                for (i = 0; i < template.apiResultParam.length; i++) {
         ...
         */
        vm.data.fun.requestProcessing = function (arg) {
            //arg status:（0：继续添加 1：快速保存，2：编辑（修改/新增））arg status: (0: continue adding 1: fast save, 2: edit (modify / add))
            var template = {
