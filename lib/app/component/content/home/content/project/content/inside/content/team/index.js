(function () {
    ...
    angular.module('danlan').config(['$stateProvider', 'RouteHelpersProvider', function ($stateProvider, helper) {
        $stateProvider.state('home.project.inside.team', {
            url: '/team',
    ...
    function homeProjectInsideTeam($scope, $rootScope, ApiManagementResource, $state, $filter, CODE) {
        ...
        vm.data.fun.setNickName = function (arg) {
            ...
            $rootScope.GroupModal(template.modal, function (callback) {
                if (callback) {
                    ApiManagementResource.Partner.SetNickName({ projectID: vm.data.interaction.request.projectID, nickName: callback.groupName, connID: arg.item.connID }).$promise.then(function (response) {
        ...
        vm.data.fun.setType = function (arg) {
            ...
            ApiManagementResource.Partner.SetType(template.request).$promise.then(function (response) {
                switch (response.statusCode) {
                    case CODE.COMMON.SUCCESS:
                        {
        ...
        vm.data.fun.add = function () {
            if (!vm.data.info.search.isDisable) {
                ...
                ApiManagementResource.Partner.Add(vm.data.interaction.request).$promise.then(function (response) {
        ...
        vm.data.fun.delete = function (arg) {
            ...
            if (bol) {
                $rootScope.EnsureModal($filter('translate')('01010118'), false, $filter('translate')('01010119'), {}, function (callback) {
                    if (callback) {
                        ApiManagementResource.Partner.Quit(template.request).$promise.then(function (response) {
                            switch (response.statusCode) {
                                case CODE.COMMON.SUCCESS:
                                    {
                ...
                $rootScope.EnsureModal($filter('translate')('01010120'), false, $filter('translate')('01010121'), {}, function (callback) {
                    if (callback) {
                        ApiManagementResource.Partner.Delete({ projectID: vm.data.interaction.request.projectID, connID: arg.item.connID }).$promise.then(function (response) {
                            switch (response.statusCode) {
                                case CODE.COMMON.SUCCESS:
                                    {
                                        if (arg.isAdmin) {
        ...
        vm.data.fun.search = function () {
            ...
            if (!!vm.data.interaction.request.username) {
                ApiManagementResource.Partner.Search(template.request).$promise.then(function (response) {
