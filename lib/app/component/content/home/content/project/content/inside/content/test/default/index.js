(function () {
    ...
    angular.module('danlan').component('homeProjectInsideTestDefault', {
    ...
    function indexController($scope, $rootScope, $state, ApiManagementResource, GroupService, CODE, $filter, HomeProject_Common_Service) {
        ...
        vm.data.fun.delete = function (status, arg) {
            ...
            $rootScope.EnsureModal($filter('translate')('01216112'), false, $filter('translate')('01216113'), {}, function (callback) {
                if (callback) {
                    ApiManagementResource.AutomatedTestCase.Delete(template.request).$promise.then(function (response) {
            ...
            $rootScope.ApiManagement_AutomatedTest_EditCaseModal(template.modal, function (callback) {
                if (callback) {
                    ...
                    switch (status) {
                        case 'add':
                            {
                                ApiManagementResource.AutomatedTestCase.Add(template.request).$promise.then(function (response) {
                                    switch (response.statusCode) {
                                        case CODE.COMMON.SUCCESS:
                                            {
                                                vm.component.batchTestObject.flag--;
                        ...
                        case 'edit':
                            {
                                ApiManagementResource.AutomatedTestCase.Edit(template.request).$promise.then(function (response) {
                                    switch (response.statusCode) {
                                        case CODE.COMMON.SUCCESS:
                                            {
                                                vm.component.batchTestObject.flag--;
