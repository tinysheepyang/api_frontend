(function () {
    ...
    angular.module('danlan').config(['$stateProvider', 'RouteHelpersProvider', function ($stateProvider, helper) {
        $stateProvider.state('home.user.basic', {
            url: '/basic',
    ...
    function indexController($scope, $rootScope, CommonResource, $state, md5, NavbarService, $filter, CODE) {
        ...
        vm.data.fun.confirm = function (arg) {
            CommonResource.User.Nickname({ nickName: vm.data.interaction.response.userInfo.userNickName }).$promise.then(function (response) {
                switch (response.statusCode) {
                    case CODE.COMMON.SUCCESS:
                        {
        ...
        vm.data.fun.changePassword = function () {
            ...
            if ($scope.passwordForm.$valid) {
                CommonResource.User.Password(template.request).$promise.then(function (response) {
                    switch (response.statusCode) {
                        case CODE.COMMON.SUCCESS:
