(function () {
    ...
    angular.module('danlan').config(['$stateProvider', 'RouteHelpersProvider', function ($stateProvider, helper) {
        $stateProvider.state('home.database.inside.table.list', {
            url: '/?tableID',
    ...
    function databaseTableListCtroller($scope, $rootScope, DatabaseResource, $state, GroupService, $filter, CODE) {
                    ...
                    $rootScope.FieldModal(template.modal, function (callback) {
                        if (callback) {
                            if (callback.status != 1) {
                                //1：为继续添加后点击关闭函数 To continue adding after clicking the close function
                                $rootScope.InfoModal(template.modal.title + $filter('translate')('01010016'), 'success');
                            }
                            vm.data.fun.init();
                        }
        ...
        vm.data.fun.delete = function (arg) {
            $rootScope.EnsureModal($filter('translate')('01010019'), false, $filter('translate')('01010020'), {}, function (callback) {
                if (callback) {
                    DatabaseResource.Field.Delete({ fieldID: arg.item.fieldID, databaseID: vm.data.interaction.request.databaseID }).$promise.then(function (response) {
                        switch (response.statusCode) {
                            case CODE.COMMON.SUCCESS:
                                {
