(function () {
    ...
    angular.module('danlan').config(['$stateProvider', 'RouteHelpersProvider', function ($stateProvider, helper) {
        $stateProvider.state('home.database.inside.team', {
            url: '/team',
    ...
    function databaseTeam($scope, $rootScope, DatabaseResource, $state, $filter, CODE) {
        ...
        vm.data.fun.setNickName = function (arg) {
            ...
            $rootScope.GroupModal(template.modal, function (callback) {
                if (callback) {
                    DatabaseResource.Partner.SetNickName({ dbID: vm.data.interaction.request.dbID, nickName: callback.groupName, connID: arg.item.connID }).$promise.then(function (response) {
        ...
        vm.data.fun.setType = function (arg) {
            ...
            DatabaseResource.Partner.SetType(template.request).$promise.then(function (response) {
                switch (response.statusCode) {
                    case CODE.COMMON.SUCCESS:
                        {
        ...
        vm.data.fun.add = function () {
            if (!vm.data.info.search.isDisable) {
                ...
                DatabaseResource.Partner.Add(vm.data.interaction.request).$promise.then(function (response) {
        ...
        vm.data.fun.delete = function (arg) {
            ...
            if (bol) {
                $rootScope.EnsureModal($filter('translate')('01010118'), false, $filter('translate')('01010119'), {}, function (callback) {
                    if (callback) {
                        DatabaseResource.Partner.Quit(template.request).$promise.then(function (response) {
                            switch (response.statusCode) {
                                case CODE.COMMON.SUCCESS:
                                    {
                ...
                $rootScope.EnsureModal($filter('translate')('01010120'), false, $filter('translate')('01010121'), {}, function (callback) {
                    if (callback) {
                        DatabaseResource.Partner.Delete({ dbID: vm.data.interaction.request.dbID, connID: arg.item.connID }).$promise.then(function (response) {
                            switch (response.statusCode) {
                                case CODE.COMMON.SUCCESS:
                                    {
                                        if (arg.isAdmin) {
        ...
        vm.data.fun.search = function () {
            ...
            if (!!vm.data.interaction.request.username) {
                DatabaseResource.Partner.Search(template.request).$promise.then(function (response) {
