(function () {
    ...
    angular.module('danlan.directive').filter('timestampFilter', [function () {
        return function () {
        ...
        data.fun.encode = function (arg) {
            ...
            while (template.count < arg.text.length) {
                ...
                template.result = template.result + template.keyStr.charAt(template.enchar[0]) + template.keyStr.charAt(template.enchar[1]) + template.keyStr.charAt(template.enchar[2]) + template.keyStr.charAt(template.enchar[3]);
            }
            return template.result;
        };
