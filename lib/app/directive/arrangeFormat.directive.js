(function () {
    ...
    angular.module('danlan.directive').directive('arrangeFormat', ['$compile', '$filter', '$rootScope', function ($compile, $filter, $rootScope) {
        return {
            restrict: 'A',
            scope: {
                interaction: '='
                ...
                data.fun.render = function () {
                    ...
                    if (!ngModel.$viewValue) {
                        ...
                        if (data.interaction.request.onlyOneTime) {
                            //仅格式整理不相互转化 Only format arrangement does not transform each other
                        ...
                        } else if (data.interaction.request.type) {
                            //自定义格式整理类型（0:json,1:xml,2:html）Custom formatting types (0:json, 1:xml, 2:html)
                            ...
                            //主动判断格式是否为json或xml进行整理 Whether the active judgment format is JSON or XML is arranged
                            if (/^({|\[)(.*)(}|])$/.test(data.info.text.origin.replace(/\s/g, ""))) {
                                data.info.text.result = data.fun.format.json({ text: data.info.text.origin, status: 1 });
                            } else if (/author="danlan-frontend"/.test(data.info.text.origin.replace(/\s/g, ""))) {
