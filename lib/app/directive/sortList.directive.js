(function (window, angular) {
            ...
            controller: ['$scope', '$attrs', '$interpolate', '$parse', function ($scope, $attrs, $interpolate, $parse) {
                ...
                if ($attrs.svGrid) {
                    ...
                    $scope.$watchCollection(function () {
                        //浅层监视（只监视对象中的第一层元素/属性，如果这些元素/属性还有嵌套属性就不在考虑范围之内）Shallow monitoring (only the first layer of elements in the object / attribute, if these elements / attributes are nested attributes are not considered within the scope)
                ...
                this.$moveUpdate = function (opts, mouse, svElement, svOriginal, svPlaceholder, originatingPart, originatingIndex) {
                    ...
                    getSortableElements(mapKey).forEach(function (se, index) {
                        if (opts.containment != null) {
                            //优化，移动开始时计算
                            if (!elementMatchesSelector(se.element, opts.containment) && !elementMatchesSelector(se.element, opts.containment + ' *')) return; // 元素不在允许的包含内
                        }
                ...
                this.addToSortableElements = function (se) {
                    //添加到排序节点集 Add to the sorted node set
                    getSortableElements(mapKey).push(se);
                };
                this.removeFromSortableElements = function (se) {
                    //从原本排序节点集移除 Remove from the original sorted node set
        ...
        return {
            ...
            controller: ['$scope', function ($scope) {
                ...
                this.getPart = function () {
                    //获取sv-root $scope.part Get sv-root $ scope.part
                    return $scope.part;
                };
        ...
        return {
            ...
            link: function ($scope, $element, $attrs, $controllers) {
                ...
                function onMousedown(e) {
                    //mouseDown函数 mouseDown function
                    touchFix(e);
                    ...
                    clone[0].reposition = function (coords) {
                        //克隆元素重定位 Clone element relocation
    ...
    .directive('svHandle', function () {
        return {
            ...
            link: function ($scope, $element, $attrs, $svCtrl) {
                if ($svCtrl) $svCtrl.handle = $element.add($svCtrl.handle); // 支持添加多级把手 Support to add multi-level handles
            }
        };
    });

    angular.element(document.head).append([//头部添加style Add style to head
    '<style>' + '.sv-helper{' + 'position: fixed !important;' + 'z-index: 99999;' + 'margin: 0 !important;' + '}' + '.sv-candidate{' + '}' + '.sv-placeholder{' +
    // 'opacity: 0;' +
    ...
    function touchFix(e) {
        //拖动位置匹配 Drag the position to match
    ...
    function getPreviousSibling(element) {
        //获取上一级元素 Get the upper level element
        element = element[0];
    ...
    function insertElementBefore(element, newElement) {
        //在被选元素内部的开头插入新节点 Insert a new node at the beginning of the selected element
    ...
    function elementMatchesSelector(element, selector) {
        //设置节点匹配选择器 Set the node matching selector
