...
(function (mod) {
  ...
  for (var i = 0; i < modes.length; ++i) CodeMirror.extendMode(modes[i], { blockCommentContinue: " * " });
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (insert == null && mode.lineComment && continueLineCommentEnabled(cm)) {
        ...
        if (found > -1) {
          ...
          if (/\S/.test(insert)) insert = null;else insert += mode.lineComment + line.slice(found + mode.lineComment.length).match(/^\s*/)[0];
        }
      }
