  ...
  function svxGenIndent(stream, state) {
    ...
    switch (state.svxCurCtlFlowChar) {
      ...
      case "|":
        ...
        if (svxchScopePrefixes[state.svxPrevCtlFlowChar]) indentUnitRq = 1; // +1 new scope
        break;
      case "M":
        // m4
        if (state.svxPrevPrevCtlFlowChar == "@") {
    ...
    hooks: {
      "\\": function (stream, state) {
        ...
        if (stream.sol() && /\\SV/.test(stream.string)) {
          curPunc = /\\SVX_version/.test(stream.string) ? "\\SVX_version" : stream.string;
          stream.skipToEnd();
          if (curPunc == "\\SV" && state.vxCodeActive) {
            state.vxCodeActive = false;
          };
          if (/\\SVX/.test(curPunc) && !state.vxCodeActive || curPunc == "\\SVX_version" && state.vxCodeActive) {
      ...
      tokenBase: function (stream, state) {
        ...
        if (state.vxCodeActive == true) {
          ...
          if (state.svxCurCtlFlowChar != vxCurCtlFlowCharValueAtStart) {
            // flow change
            vxIndent = svxGenIndent(stream, state);
            state.vxIndentRq = vxIndent;
          }
