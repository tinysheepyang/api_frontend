  ...
  CodeMirror.defineMode("r", function (config) {
    ...
    return {
      ...
      token: function (stream, state) {
        ...
        } else if (curPunc == "[") push(state, "]", stream);else if (curPunc == "block") push(state, "block", stream);else if (curPunc == ctype) pop(state);
      ...
      indent: function (state, textAfter) {
        ...
        if (ctx.type == "block") return ctx.indent + (firstChar == "{" ? 0 : config.indentUnit);else if (ctx.align) return ctx.column + (closing ? 0 : 1);else return ctx.indent + (closing ? 0 : config.indentUnit);
      },
