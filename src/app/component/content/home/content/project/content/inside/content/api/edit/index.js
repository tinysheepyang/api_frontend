(function () {
    ...
    indexController.$inject = ['$cookies', '$scope', '$rootScope', 'ApiManagementResource', '$state', 'GroupService', '$filter', 'CODE', 'HTTP_CONSTANT', 'HTML_LAZYLOAD'];

    function indexController($cookies, $scope, $rootScope, ApiManagementResource, $state, GroupService, $filter, CODE, HTTP_CONSTANT, HTML_LAZYLOAD) {
        var vm = this;
        vm.data = {
            constant: {
            ...
            template.promise.then(function (response) {
                switch (response.statusCode) {
                    case CODE.COMMON.SUCCESS: {
                        ...
                        vm.data.interaction.response.apiInfo.userID = $cookies.get('id');
                        window.scrollTo(0, 0);
                        vm.data.info.menu = 0;
            ...
            if (template.cache.group) {
                if (vm.data.info.reset.apiID) {
                    ApiManagementResource.Api.Detail({
                        ...
                        switch (response.statusCode) {
                            case CODE.COMMON.SUCCESS: {
                                response = angular.fromJson(response.apiInfo);
                                vm.data.interaction.response.apiInfo = response.baseInfo;
                                vm.data.interaction.response.apiInfo.mockConfig = response.mockInfo ? (response.mockInfo.mockConfig || {
                                    type: 'object',
                                    rule: ''
                                }) : {
                                    type: 'object',
                                ...
                                vm.data.interaction.response.apiInfo.apiHeader = response.headerInfo;
                                vm.data.interaction.response.apiInfo.apiRequestParam = response.requestInfo;
                                vm.data.interaction.response.apiInfo.apiResultParam = response.resultInfo;
                                ...
                                vm.data.interaction.response.apiInfo.mockRule = response.mockInfo ? response.mockInfo.rule : [];
                                ...
                                if (!vm.data.interaction.response.apiInfo.topParentGroupID) {
                                    if (vm.data.interaction.response.apiInfo.parentGroupID) {
                                        vm.data.interaction.response.apiInfo.childGroupID = response.baseInfo.groupID;
                                    ...
                                    vm.data.interaction.response.apiInfo.grandSonGroupID = response.baseInfo.groupID;
                                    vm.data.interaction.response.apiInfo.childGroupID = response.baseInfo.parentGroupID;
                                    vm.data.interaction.response.apiInfo.groupID = response.baseInfo.topParentGroupID;
                                }
