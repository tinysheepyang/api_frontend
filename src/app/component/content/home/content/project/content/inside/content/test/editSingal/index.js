            ...
            switch (vm.data.info.status) {
                ...
                case 'add': {
                    ...
                    if (template.cache.apiInfo) {
                        ...
                        ApiManagementResource.Api.Single(template.request).$promise.then(function (response) {

                            vm.data.interaction.response.caseInfo.caseData.requestType = (response.apiInfo.baseInfo.apiRequestParamType || 0).toString();
                            vm.data.interaction.response.caseInfo.caseData.params = response.apiInfo.requestInfo;
                            vm.data.interaction.response.caseInfo.caseData.raw = response.apiInfo.baseInfo.apiRequestRaw;
                ...
                case 'edit': {
                    ...
                    $rootScope.global.ajax.Info_AutomatedTestCaseSingle.$promise.then(function (response) {
                        ...
                        switch (vm.data.interaction.response.caseInfo.matchType) {
                            ...
                            default: {
                                vm.data.interaction.request.matchTextarea = angular.toJson(vm.data.interaction.response.caseInfo.matchRule);
                                vm.data.fun.last('response', {
                                    $last: true
                                });
                            }
                        }
                        try {
                            ...
                            vm.data.interaction.response.caseInfo.caseData = {
                                "auth": {
                                    "status": "0"
                                },
                                "headers": [],
