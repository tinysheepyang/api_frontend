(function () {
    ...
    function indexController($scope, $rootScope, ApiManagementResource, CODE, $state, Cache_CommonService, HomeProject_Common_Service, $filter) {
        ...
        vm.data.fun.detail = function (status, arg) {
            ...
            if (arg.item.matchRule == "" || arg.item.matchRule == null) {
                arg.item.matchRule = [];
            } else {
                arg.item.matchRule = angular.fromJson(arg.item.matchRule);
            }
        ...
        vm.data.fun.test = function (status, arg) {
            switch (status) {
                case 'all': {
                    ...
                    vm.data.interaction.batchTestObject.statusObject.caseID = arg.caseID;
                    vm.data.interaction.batchTestObject.statusObject.envID = vm.data.service.home.envObject.object.total.envID;
                    vm.data.interaction.batchTestObject.statusObject.projectID = vm.data.interaction.request.projectID;
                ...
                case 'singal': {
                    ...
                    arg.item.envID = vm.data.service.home.envObject.object.total.envID;
            ...
            if (status == 'singal') {
                ApiManagementResource.AutomatedTestCase.Execute(arg.item).$promise.then(function (data) {
                    vm.data.interaction.batchTestObject.output = data.testResult;
                    vm.data.interaction.response.query[arg.$index].testing = false;
                })
            } else {
                ApiManagementResource.AutomatedTestCase.AllExecute(vm.data.interaction.batchTestObject.statusObject).$promise.then(function (data) {
                    vm.data.interaction.batchTestObject.output = data.testResult;
                    vm.data.info.statusObject.testing = false;
