(function () {
    ...
    homeProjectInsideApiTestController.$inject = ['$cookies', '$window', '$scope', '$rootScope', 'ApiManagementResource', '$state', 'Cache_CommonService', 'HomeProject_Common_Service', 'HomeProjectDefaultApi_Service', '$filter', 'CODE', 'HTTP_CONSTANT'];

    function homeProjectInsideApiTestController($cookies, $window, $scope, $rootScope, ApiManagementResource, $state, Cache_CommonService, HomeProject_Common_Service, HomeProjectDefaultApi_Service, $filter, CODE, HTTP_CONSTANT) {
        var vm = this;
        vm.data = {
            service: {
        ...
        vm.data.fun.testList.enter = function (arg) {
            ...
            if (/image\/(jpg|jpeg|png|gif)/ig.test(JSON.stringify(arg.item.resultInfo.headers))) {
                template.img.html = '<img style="width:100%;" author="danlan-frontend" src="' + arg.item.resultInfo.body + '"/>';
            }
        ...
        vm.data.assistantFun.init = function () {
            $scope.$emit('$WindowTitleSet', {
                list: [$filter('translate')('012100055') + vm.data.interaction.response.apiInfo.baseInfo.apiName, $filter('translate')('012100164'), $state.params.projectName, 'API开发管理']
            });
            ...
            $scope.$emit('$translateferStation', {
                ...
                data: {
                    status: 2,
                    param: angular.toJson(vm.data.service.home.envObject.object.model),
                    header: 'headers',
        ...
        vm.data.fun.init = function () {
            ...
            $rootScope.global.ajax.Query_Env.$promise.then(function (response) {
                vm.data.info.env = response.envList || [];
                $scope.$emit('$translateferStation', {
                    state: '$EnvInitReady',
                    data: {
                        status: 0
            ...
            if (template.cache.testInfo) {
                ...
                $scope.$emit('$WindowTitleSet', {
                    list: [$filter('translate')('012100055') + vm.data.interaction.response.apiInfo.baseInfo.apiName, $filter('translate')('012100036'), $state.params.projectName, 'API开发管理']
                });
                $scope.$emit('$translateferStation', {
                    state: '$EnvInitReady',
                    data: {
                        status: 2,
                        reset: 1,
                        resetInfo: vm.data.info.template.envModel,
                        param: angular.toJson(vm.data.service.home.envObject.object.model),
                        header: 'headers',
                        uri: 'URL'
                    }
            ...
            console.log('vm.data.service.home.envObject.object.model.params----', vm.data.service.home.envObject.object.model.params);
        }
        vm.data.fun.init();
        vm.$onInit = function () {
            ...
            $scope.$on('$stateChangeStart', function () {
                ...
                vm.data.service.home.apiTestObject.fun.set({
                    object: {
                        reset: vm.data.info.template.envModel,
                        apiInfo: vm.data.interaction.response.apiInfo,
                        message: vm.data.service.home.envObject.object.model,
                        result: vm.data.info.response,
                        format: vm.data.info.format
                    }
                });
