(function () {
    ...
    indexController.$inject = ['$cookies', '$scope', '$rootScope', '$state', 'ApiManagementResource', 'GroupService', 'CODE', '$filter', 'HomeProject_Common_Service'];

    function indexController($cookies, $scope, $rootScope, $state, ApiManagementResource, GroupService, CODE, $filter, HomeProject_Common_Service) {
        var vm = this;
        vm.data = {
            service: {
                group: GroupService,
        ...
        vm.data.fun.test = function () {
            vm.data.interaction.request.envID = vm.data.service.home.envObject.object.total.envID;

            ApiManagementResource.AutomatedTestCase.projectExecute(vm.data.interaction.request).$promise.then(function (data) {
                ...
                vm.component.reportObject.testing = false;

                $rootScope.$broadcast("allCaseResult", {
                    input: data.allCaseResult
                });
