            ...
            $rootScope.global.ajax.Mock_Api.$promise.then(function (response) {
                switch (response.statusCode) {
                    case CODE.COMMON.SUCCESS: {
                        vm.data.interaction.response.mockInfo = angular.fromJson(response.result);
                        vm.data.interaction.response.mockInfo.mockRule = angular.fromJson(response.result.mockRule);
                        ...
                        $scope.$emit('$WindowTitleSet', {
                            list: ['[Mock]' + vm.data.interaction.response.mockInfo.apiName, $filter('translate')('012100164'), $state.params.projectName, 'API开发管理']
                        });
                        break;
                    }
                }
