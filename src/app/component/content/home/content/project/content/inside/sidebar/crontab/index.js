(function () {
     ...
     *
     * @function [crontab侧边栏相关js] [crontab sidebar related js]
     * @version  3.2.0
    ...
    angular.module('danlan')
        .component('homeProjectInsideCrontabSidebar', {
            templateUrl: 'app/component/content/home/content/project/content/inside/sidebar/crontab/index.html',
            bindings: {
                powerObject: '<'
            },
    ...
    indexController.$inject = ['$cookies', 'ERROR_WARNING', '$scope', 'ApiManagementResource', '$state', 'CODE', '$rootScope', 'GroupService', 'Group_AmsCommonService'];

    function indexController($cookies, ERROR_WARNING, $scope, ApiManagementResource, $state, CODE, $rootScope, GroupService, Group_AmsCommonService) {
        var vm = this;
        vm.data = {
            service: {
        ...
        vm.data.fun.init = function () {
            var template = {
                request: {
                    ...
                    taskID: vm.data.interaction.request.taskID,
            ...
            if ($state.current.name.indexOf('edit') > -1 || $state.current.name.indexOf('history') > -1) {
                vm.data.info.sidebarShow = false;
            } else {
                vm.data.info.sidebarShow = true;
            }
            ApiManagementResource.CrontabGroup.Query(template.request).$promise.then(function (response) {
                switch (response.statusCode) {
                    case CODE.COMMON.SUCCESS: {
                        ...
                        if ($state.current.name.indexOf('edit') > -1 || $state.current.name.indexOf('result') > -1) {
        ...
        vm.data.fun.sort.confirm = function () {
            var template = {
                input: {
                    ...
                    resource: ApiManagementResource.CrontabGroup.Sort,
                    callback: null
                }
            }
        ...
        vm.data.fun.click = function (status, arg) {
            ...
            $state.go('home.project.inside.crontab.list', template.uri);
        }
        vm.data.fun.edit = function (status, arg) {
            ...
            var template = {
                options: {
                    ...
                    resource: ApiManagementResource.CrontabGroup,
        ...
        vm.data.fun.delete = function (arg) {
            ...
            var template = {
                modal: {
                    title: '删除分组',
                    message: '删除分组后，该分组下的crontab将全部移入接口回收站，该操作无法撤销，确认删除？'
                }
            }
            $rootScope.EnsureModal(template.modal.title, false, template.modal.message, {}, function (callback) {
                if (callback) {
                    ApiManagementResource.CrontabGroup.Delete({
                        projectID: vm.data.interaction.request.projectID,
