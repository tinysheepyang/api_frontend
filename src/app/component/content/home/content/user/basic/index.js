    ...
    function indexController($scope, $rootScope, CommonResource, $state, md5, NavbarService, $filter, CODE) {
        ...
        vm.data.fun.init = function () {
            ...
            $scope.$emit('$WindowTitleSet', {
                list: [$filter('translate')('0130'), $filter('translate')('01313')]
            });
        ...
        vm.data.fun.confirm = function (arg) {
            CommonResource.User.Nickname({
                    nickName: vm.data.interaction.response.userInfo.userNickName
                }).$promise
                .then(function (response) {
                    switch (response.statusCode) {
                        case CODE.COMMON.SUCCESS: {
                            $scope.$emit('$translateferStation', {
                                state: '$EoNavbarChangeUser',
                                data: vm.data.interaction.response.userInfo.userNickName
                            });
                        }
                        case '130009': {
        ...
        vm.data.fun.changePassword = function () {
            var template = {
                request: {
                    username: vm.data.interaction.response.userInfo.userNickName,
