(function () {
    'use strict';
    /**
     * @name danlan open source，danlan开源版本
     ...
     *
     * 注意！danlan开源版本遵循GPL V3开源协议，仅供用户下载试用，禁止“一切公开使用于商业用途”或者“以danlan开源版本为基础而开发的二次版本”在互联网上流通。
    ...
    angular.module('danlan')
        .config(['$stateProvider', 'RouteHelpersProvider', function ($stateProvider, helper) {
            $stateProvider
                ...
                }).state('register.forget', {
                    url: '/forget',
                    auth: true,
                    template: '<forget-default></forget-default>'
                });
        }])
        ...
        .component('forgetDefault', {
            templateUrl: 'app/component/content/register/content/default/forget.html',
            controller: registerDefaultController
        })
    ...
    function registerDefaultController($scope, $rootScope, CommonResource, $state, md5, $filter, CODE) {
        ...
        vm.data = {
            info: {
                ...
                emailalert: $filter('translate')('03026')
            },
            interaction: {
                request: {
                    username: '',
         ...
         * @function [查重功能函数] [Check repeat]
         */
        vm.data.fun.checkemail = function () {
            var template = {
                request: {
                    username: vm.data.interaction.request.email
                }
            }
            if (template.request.email != '') {
                CommonResource.GuestRegister.Check(template.request)
                    .$promise.then(function (response) {
                        switch (response.statusCode) {
                            case CODE.COMMON.SUCCESS: {
                                vm.data.info.unavailable = false;
                                vm.data.info.emailalert = $filter('translate')('03022');
                                break;
                            }
                            case CODE.USER.EXIST: {
                                vm.data.info.unavailable = true;
                                vm.data.info.emailalert = $filter('translate')('03023');
                                break;
                            }
                            default: {
                                vm.data.info.unavailable = true;
                                vm.data.info.alert = $filter('translate')('03024');
                                break;
                            }
                        }
                    })
        ...
        /**
         * @function [找回密码发送邮件功能函数] [send]
         */
        vm.data.fun.send = function () {
            var template = {
                request: {
                    email: vm.data.interaction.request.email
                }
            ...
            if (template.request.email != '') {
                CommonResource.GuestRegister.Forget(template.request)
                    .$promise.then(function (response) {
                        switch (response.statusCode) {
                            case CODE.COMMON.SUCCESS: {
                                vm.data.info.unavailable = false;
