(function () {
    ...
    angular.module('danlan.modal')
        ...
        .controller('ApiManagement_AutomatedTestCase_QiuckAddTestCaseModalCtrl', ApiManagement_AutomatedTestCase_QiuckAddTestCaseModalCtrl)

        .controller('ApiManagement_AutomatedCrontab_RelevanceTestCaseModalCtrl', ApiManagement_AutomatedCrontab_RelevanceTestCaseModalCtrl)
        ...
        data.fun.init = (function () {
            ...
            if (window.localStorage['ONLINEINFO']) {
                try {
                    ...
                    $scope.data.interaction.request.password = $filter('aesDecryptFilter')(template.interaction.request.password);
        ...
        $scope.data.fun.confirm = function () {
            var template = {
                storage: {
                    ...
                    password: $filter('aesEncryptFilter')($scope.data.interaction.request.password)
                },
                request: {
                    userCall: $scope.data.interaction.request.userCall,
                    password: md5.createHash($scope.data.interaction.request.password),
    ...
    function ApiManagement_AutomatedTest_BindModalCtrl($scope, $rootScope, $uibModalInstance, Cache_CommonService, input, $filter) {
        ...
        $scope.data.fun.changeSingal = function (arg) {
            ...
            $scope.data.info.current.matchRule = angular.fromJson($scope.data.info.current.matchRule);
            console.log('$scope.data.info.current.matchRule-----+++++', $scope.data.info.current.matchRule, typeof ($scope.data.info.current.matchRule));
    ...
    /**
     * @function [自动化测试加载TestCase管理弹窗] [Automate test loading API management popups]
     * @service  $scope [注入作用域服务] [Injection scope service]
     * @service  $rootScope [注入根作用域服务] [Injection rootScope service]
     * @service  $uibModalInstance [注入$uibModalInstance服务] [Injection $uibModalInstance service]
     * @service  Cache_CommonService [注入Cache_CommonService服务] [Injection Cache_CommonService service]
     ...
     * @param    {[obj]}   input [参数详情 Parameter details]
     */
    ApiManagement_AutomatedTestCase_QiuckAddTestCaseModalCtrl.$inject = ['$scope', 'Cache_CommonService', 'Group_AmsCommonService', '$rootScope', '$uibModalInstance', 'input'];

    function ApiManagement_AutomatedTestCase_QiuckAddTestCaseModalCtrl($scope, Cache_CommonService, Group_AmsCommonService, $rootScope, $uibModalInstance, input) {
        $scope.data = {
            input: {
                projectList: angular.copy(input.projectList)
            },
        ...
        data.fun.getTestCaseList = function () {
            var template = {
                promise: null
            }
            template.promise = input.resource.AutomatedTestCase.Query(input.request).$promise;
            template.promise.then(function (response) {
                $scope.data.interaction.response.testCaseList = response.caseList || [];
            });
            return template.promise;
        }
        data.fun.init = (function () {
            var template = {
                cache: {
                    testCaseList: data.service.cache.get('testCaseList')
                },
                apiPromise: null
            }
            $scope.data.component.groupCommonObject = {
            ...
            template.cache.group = data.service.cache.get('testCaseGroup');
            if (template.cache.group) {
                $scope.data.interaction.response.groupList = template.cache.group;
            } else {
                input.resource.AutomatedTestGroup.Query(input.request).$promise.then(function (response) {
                    $scope.data.interaction.response.groupList = data.service.defaultCommon.sort.init(response);
                    data.service.cache.set($scope.data.interaction.response.groupList, 'testCaseGroup');
                })
            }

            if (template.cache.testCaseList) {
                $scope.data.interaction.response.testCaseList = template.cache.testCaseList;
            } else {
                template.apiPromise = data.fun.getTestCaseList();
                template.apiPromise.finally(function () {
                    data.service.cache.set($scope.data.interaction.response.testCaseList, 'testCaseList');
                })
            }
        })()
    }
    ...
    /**
     * @function [定时任务加载TestCase关联弹窗] [Automate test loading API management popups]
     * @service  $scope [注入作用域服务] [Injection scope service]
     * @service  $rootScope [注入根作用域服务] [Injection rootScope service]
     * @service  $uibModalInstance [注入$uibModalInstance服务] [Injection $uibModalInstance service]
     * @service  Cache_CommonService [注入Cache_CommonService服务] [Injection Cache_CommonService service]
     ...
     * @param    {[obj]}   input [参数详情 Parameter details]
     */
    ApiManagement_AutomatedCrontab_RelevanceTestCaseModalCtrl.$inject = ['$scope', 'Cache_CommonService', 'Group_AmsCommonService', '$rootScope', '$uibModalInstance', 'input'];

    function ApiManagement_AutomatedCrontab_RelevanceTestCaseModalCtrl($scope, Cache_CommonService, Group_AmsCommonService, $rootScope, $uibModalInstance, input) {
        $scope.data = {
            input: {
                projectList: angular.copy(input.projectList)
            },
        ...
        data.fun.getTestCaseList = function () {
            ...
            template.promise = input.resource.Crontab.QueryTaskCase(input.request).$promise;
            template.promise.then(function (response) {
                $scope.data.interaction.response.testCaseList = response.taskCaseList || [];
            });
            return template.promise;
        }
        data.fun.init = (function () {
