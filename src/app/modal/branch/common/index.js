(function () {
    ...
    angular.module('danlan.modal')
        ...
        .controller('CrontabResumeModalCtrl', CrontabResumeModalCtrl)

        .controller('CrontabPauseModalCtrl', CrontabPauseModalCtrl)

        .controller('CrontabExecuteModalCtrl', CrontabExecuteModalCtrl)
        ...
        .controller('CommonCrontabInputModalCtrl', CommonCrontabInputModalCtrl)
    ...
    CrontabResumeModalCtrl.$inject = ['$scope', '$uibModalInstance', '$filter', 'title', 'necessity', 'info', 'btn'];
    /**
     * @function [定时任务启动确认弹窗] [confirm]
     * @service  $scope [注入作用域服务] [Injection scope service]
     * @service  $uibModalInstance [注入$uibModalInstance服务] [Injection $uibModalInstance service]
     * @service  $filter [注入过滤器服务] [Injection filter service]
     * @param    {[string]}   title [弹窗标题 Pop-up title]
     ...
     * @param    {[number]}   btn [按钮类型 Button type]
     */
    function CrontabResumeModalCtrl($scope, $uibModalInstance, $filter, title, necessity, info, btn) {

        $scope.title = title;
        $scope.necessity = necessity;
        $scope.info = {
            message: info || $filter('translate')('515'),
            btnType: btn.btnType || 0, //0：warning 1：info,2:success,
            btnMessage: btn.btnMessage || $filter('translate')('514')
        }
        $scope.ok = function () {
            if ($scope.sureForm.$valid || !$scope.necessity) {
                $uibModalInstance.close(true);
    ...
    CrontabPauseModalCtrl.$inject = ['$scope', '$uibModalInstance', '$filter', 'title', 'necessity', 'info', 'btn'];
    /**
     * @function [定时任务暂停确认弹窗] [confirm]
     * @service  $scope [注入作用域服务] [Injection scope service]
     * @service  $uibModalInstance [注入$uibModalInstance服务] [Injection $uibModalInstance service]
     * @service  $filter [注入过滤器服务] [Injection filter service]
     * @param    {[string]}   title [弹窗标题 Pop-up title]
     ...
     * @param    {[number]}   btn [按钮类型 Button type]
     */
    function CrontabPauseModalCtrl($scope, $uibModalInstance, $filter, title, necessity, info, btn) {

        $scope.title = title;
        $scope.necessity = necessity;
        $scope.info = {
            message: info || $filter('translate')('513'),
            btnType: btn.btnType || 0, //0：warning 1：info,2:success,
            btnMessage: btn.btnMessage || $filter('translate')('512')
        }
        $scope.ok = function () {
            if ($scope.sureForm.$valid || !$scope.necessity) {
                $uibModalInstance.close(true);
    ...
    CrontabExecuteModalCtrl.$inject = ['$scope', '$uibModalInstance', '$filter', 'title', 'necessity', 'info', 'btn'];
    /**
     * @function [定时任务调试确认弹窗] [confirm]
     * @service  $scope [注入作用域服务] [Injection scope service]
     * @service  $uibModalInstance [注入$uibModalInstance服务] [Injection $uibModalInstance service]
     * @service  $filter [注入过滤器服务] [Injection filter service]
     * @param    {[string]}   title [弹窗标题 Pop-up title]
     ...
     * @param    {[number]}   btn [按钮类型 Button type]
     */
    function CrontabExecuteModalCtrl($scope, $uibModalInstance, $filter, title, necessity, info, btn) {

        $scope.title = title;
        $scope.necessity = necessity;
        $scope.info = {
            message: info || $filter('translate')('516'),
            btnType: btn.btnType || 0, //0：warning 1：info,2:success,
            btnMessage: btn.btnMessage || $filter('translate')('517')
        }
        $scope.ok = function () {
            if ($scope.sureForm.$valid || !$scope.necessity) {
                $uibModalInstance.close(true);
    ...
    function ExportModalCtrl($scope, $uibModalInstance, ApiManagementResource, $filter, $state, CODE, $rootScope, input) {
        ...
        $scope.data.fun.dumpDirective = function (arg) {
            ...
            switch (arg.switch) {
                ...
                case 'export-report': {
                    template.request.taskID = $scope.data.input.request.taskID;
                    template.request.identification = $scope.data.input.request.identification;
                    template.request.key = $scope.data.input.request.key;
                    template.request.type = $scope.data.input.request.type;

                    template.promise = ApiManagementResource.Crontab.Download(template.request).$promise;
                    break;
                }
                case "0": {
    ...
    GroupModalCtrl.$inject = ['$cookies', '$scope', '$uibModalInstance', '$timeout', 'CODE', 'input'];
    /**
     ...
     */
    function GroupModalCtrl($cookies, $scope, $uibModalInstance, $timeout, CODE, input) {
        var code = CODE.COMMON.SUCCESS;
        var vm = this;
        $scope.title = input.title;
    ...
    function UpdateModalCtrl($scope, $rootScope, $uibModalInstance, ApiManagementResource, $filter, $state, $http, CODE) {
        ...
        var request = {
            method: 'GET',
            url: 'https://api.danlan.com/openSource/JavaVersionUpdate/checkout',
        };
    ...
    CommonCrontabInputModalCtrl.$inject = ['$scope', '$uibModalInstance', '$filter', 'title', 'desc', 'info', 'input'];
    /**
     * @function [单一输入框弹窗] [Single input box]
     * @service  $scope [注入作用域服务] [Injection scope service]
     * @service  $uibModalInstance [注入$uibModalInstance服务] [Injection $uibModalInstance service]
     ...
     * @param    {[string]}   title [弹窗标题 Pop-up title]
     * @param    {[obj]}   desc [描述信息 description information]
     * @param    {[obj]}   info [crontab信息 API information]
     * @param    {[string]}   input [按钮类型 Button type]
     */
    function CommonCrontabInputModalCtrl($scope, $uibModalInstance, $filter, title, desc, info, input) {
        $scope.title = title;
        $scope.info = {
            desc: desc,
            message: info,
            btnType: input.btnType || 0, //0：warning 1：info,
            btnMessage: input.btnMessage || $filter('translate')('511'),
            placeholder: $filter('translate')('012100174'),
        }
        $scope.data = {
            input: input
        }
